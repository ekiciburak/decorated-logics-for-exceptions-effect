...
Module Make(Import M: Prerequistes.T).
 ...
 Definition permut {X Y} : term (X+Y) (Y+X) := copair in2 in1.
...
Lemma s_rcopair_eq: forall X Z Y (f1: term X Y) (f2: term X Z), PPG f2 -> rcopair f1 f2 o in1 == f1.
Proof.
    intros X Y' Y f1 f2 H0. unfold rcopair, permut. rewrite <- assoc.
    cut (copair in2 in1 o in1 == (@in2 Y' Y)).
      intro H1. rewrite H1.
...
Lemma w_rcopair_eq: forall X Z Y (f1: term X Y) (f2: term X Z), PPG f2 -> rcopair f1 f2 o in2 ~ f2.
Proof.
...
Lemma lcopair_u: forall X Y Y' (f1 f2: term X (Y' + Y)),
  (f1 o in1 ~ f2 o in1) /\ (f1 o in2 == f2 o in2) -> f1 == f2.
Proof.
    intros X Y Y' f1 f2 (H0&H1). apply eeffect.
    (* f 1 ◦ [ ] ≡ f 2 ◦ [ ] *)
    cut((@in2 Y' Y) o (@empty Y) == (@empty (Y' + Y))).
      intro H2. rewrite <- H2.
...
Proof.
     intros X Y Y' f1 f2 (H0&H1). apply eeffect.
     (* f 1 ◦ [ ] ≡ f 2 ◦ [ ] *)
     cut((@in1 Y' Y) o (@empty Y') == (@empty (Y' + Y))).
       intro H2. rewrite <- H2.
